var _ = require('underscore'),
    async = require('async'),
    child_process = require('child_process'),
    handlebars = require('handlebars'),
    tempfile = require('tempfile'),
    jsonfile = require('jsonfile'),
    express = require('express'),
    body_parser = require('body-parser');

function server(config) {
  var defaults = {
    'port': 3000,
    'verbose': false
  }
  this.config = _.extend(_.clone(defaults), config || {});
  if (this.config.verbose) {
    console.log(config);
  }

  var queues = {};
  _.each(this.config.repos, function (template, url) {
    queues[url] = async.queue(function (payload, callback) {
      var jsonFile = tempfile('.json');
      jsonfile.writeFileSync(jsonFile, payload);
      payload.file = jsonFile;
      var command = handlebars.compile(template)(payload);
      if (this.config.verbose) {
        console.log("Running " + command);
      }
      child_process.exec(command, function (err) {
        if (err && this.config.verbose) {
          console.log("Command failed: " + err);
        }
        callback();
      });
    }, 1);
  });

  var branchPattern = /^refs\/heads\/(\w+)$/;
  this.app = express();
  app.use(body_parser.urlencoded({extended: true}));
  app.post(/.*/, function (req, res) {
    var payload = decodeURIComponent(req.body.payload);
    if (this.config.verbose) {
      console.log(payload);
    }

    try {
      payload = JSON.parse(payload);
    } catch (err) {
      if (this.config.verbose) {
        console.log("Error parsing payload: " + err);
      }
      res.status(500).send();
      return;
    }
    var url = payload.repository.html_url || payload.repository.url;
    if (!(url in this.config.repos)) {
      if (this.config.verbose) {
        console.log("Repository not registered: " + url);
      }
      res.status(404).send();
      return;
    }
    var branch = payload.branch || branchPattern.exec(payload.ref)[1];
    if (!branch) {
      if (this.config.verbose) {
        console.log("Couldn't parse branch from payload");
      }
      res.status(500).send();
      return;
    }
    var normalizedInfo = {
      commit: payload.commit || payload.head_commit.id,
      branch: branch,
      message: payload.message || payload.head_commit.message,
      author_name: payload.author_name || payload.head_commit.author.name,
      author_email: payload.author_email || payload.head_commit.author.email,
      timestamp: payload.committed_at || payload.head_commit.timestamp
    }
    queues[payload.repository.url].push(normalizedInfo);
    res.status(200).send();
  });

  this.start = function() {
    app.listen(this.config.port);
  }

  return this;
}

if (require.main === module) {
  var argv = require('minimist')(process.argv.slice(2)),
      fs = require('fs');
  server(JSON.parse(fs.readFileSync(argv._[0]))).start();
}

exports = module.exports = server

// vim: ts=2:sw=2:et
